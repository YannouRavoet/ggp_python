...
class GamePlayerBase(HTTPServer, ABC):
...
class GamePlayer(GamePlayerBase):
    def handle_message_play(self, msg_args):
        ...
        action = self.player_play(jointaction, timeout=self.clock_left())
...
class GamePlayerII(GamePlayer, ABC):
    def handle_message_play(self, msg_args):
        ...
        next_action = self.player_play(action, percepts, timeout=self.clock_left())
...
class GamePlayerSTOII(GamePlayer, ABC):
    def handle_message_play(self, msg_args):
    ...
    def handle_message_stop(self, msg_args):
        ...
        outcome = Action(self.role, outcome)
        percepts = Percepts([Percept(self.role, percept) for percept in percepts])
        goal_value = self.player_stop(action, outcome, percepts, timeout=self.clock_left())
        self.matchInfo.add_result(self.role, goal_value)
        return Message(MessageType.DONE)
